...
def main():
    ...
    parser.add_argument('--data_name', type=str, default='semeval')
    parser.add_argument('--in_dir', type=str, default='../dataset/')
    parser.add_argument('--out_dir', type=str, default='../json_new_se/')
    args = vars(parser.parse_args())

    in_dir = Path(args['in_dir']+args['data_name'])
    ...
    split_parts(in_dir / 'train.json', 0.5, ['train-0.5.json', 'raw-0.5.json'])
    print('Sample from data...')
    sample_from_data(in_dir / 'train-0.5.json',
                     [0.6, 0.2, 0.1])  # actually sampling 10% and 5% of the original training data
    sample_from_data(in_dir / 'raw-0.5.json',
                     [0.6, 0.2, 0.1])  # actually sampling 10% and 5% of the original training data
